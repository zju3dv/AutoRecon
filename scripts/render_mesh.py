...
from nerfstudio.cameras.camera_paths import get_path_from_json, get_path_from_npz, get_spiral_path
...
def _render_trajectory_video(
    ...
    merge_type: Literal["half", "concat", "separate"] = "half",
) -> None:
    ...
    vis = o3d.visualization.VisualizerWithKeyCallback()  # pyright: ignore
...
class RenderTrajectory:
    ...
    output_format: Literal["images", "video"] = "video"
    merge_type: Literal["half", "concat", "separate"] = "concat"

    load_config: Optional[Path] = None
    data: SDFStudioDataParserConfig = SDFStudioDataParserConfig()
    num_interpolate_views: int = 300
    ...
    def main(self) -> None:
        ...
        if self.traj == "filename":
            ...
            camera_path = vis_utils.interpolate_trajectory(cameras=outputs.cameras, num_views=self.num_interpolate_views)
            seconds = camera_path.size / 24
        elif self.traj == "spiral":
            outputs = self.data.setup()._generate_dataparser_outputs()
            camera_path = get_spiral_path(camera=outputs.cameras, steps=self.num_interpolate_views, radius=1.0)
            seconds = camera_path.size / 24
        else:
